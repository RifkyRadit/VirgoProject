...
protocol ListTrendingViewInput: AnyObject {
    func setContenListTrending(withData data: [MoviesResult])
}

protocol ListTrendingViewDelegate: AnyObject {
    func didSelectedTrendingItem(withIndex index: Int)
}

class ListTrendingView: UIView {
    
    weak var delegate: ListTrendingViewDelegate?
    private var collectionHeight: CGFloat = 0
    
    private var listData: [MoviesResult] = [] {
        didSet {
            self.reloadData()
        }
    }
...
extension ListTrendingView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        delegate.didSelectedTrendingItem(withIndex: indexPath.row)
    }
    
}

extension ListTrendingView: UICollectionViewDelegateFlowLayout {
...
extension ListTrendingView: ListTrendingViewInput {
    func setContenListTrending(withData data: [MoviesResult]) {
        self.listData = data
    }
}
